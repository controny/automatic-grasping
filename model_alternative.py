...
def alexnet_v2(images, is_training, num_classes, lmbda):
    ...
    return net
def create_loss(logits, theta_labels, class_labels):
    """
    :param logits: should be shape of [batch_size, 18]
    :param theta_labels: each denoted by an one-hot vector [batch_size, 18]
    :param class_labels: each denoted by 0 or 1, should be converted to float [batch_size, 1]
    :return: reduce mean of loss and accuracy for a batch
    """
    class_labels = tf.cast(tf.squeeze(class_labels), tf.int32)
    theta_acted = tf.reduce_sum(tf.multiply(logits, theta_labels), axis=1, name='theta_acted')  # [batch_size, 1]
    #theta_acted = tf.Print(theta_acted,[theta_acted, theta_acted.shape],message='theta_acted:', summarize= 128*2)
    #class_labels = tf.Print(class_labels,[class_labels, class_labels.shape],message='class_labels:', summarize= 128)

    sig_op = tf.clip_by_value(theta_acted, 0.001, 0.999, name='clipped_sigmoid')
    #sig_op = tf.Print(sig_op,[sig_op, sig_op.shape],message='sig_op:', summarize= 128)
    ...
    #conf = tf.Print(conf,[conf, conf.shape],message='conf:', summarize= 128)
    accuracy = tf.reduce_mean(tf.to_float(conf))
    #accuracy = tf.Print(accuracy,[accuracy, accuracy.shape],message='accuracy:')
...
def get_num_correctness(logits, theta_labels, class_labels):
    ...
    :param logits: should be shape of [batch_size, 18]
    :param theta_labels: each denoted by an one-hot vector [batch_size, 18]
    :param class_labels: each denoted by 0 or 1 [batch_size, 1]
